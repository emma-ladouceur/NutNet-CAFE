...
View(ctl)
#max values only
#ctl2<-ctl %>% group_by(site_code) %>% top_n(1, year.y)


npk<-p.all[p.all$trt.x %in% c('NPK'),]
View(npk)
#max values only
#npk2<-npk %>% group_by(site_code) %>% top_n(1, year.y)



??leap.zig
theme_update(panel.border = element_rect(linetype = "solid", colour = "black"))
p1 <- leap.zig(ctl,type='cafe',xlim=c(0,15),ylim=c(0,700),standardize = FALSE,raw.points = F)+ 
  annotate("text", x = mean(ctl$x.rich), y = mean(ctl$x.func), 
           label = "*",size=8)+ggtitle('Control 0 Control N')+theme_classic()
p2 <- leap.zig(npk,type='cafe',xlim=c(0,15),ylim=c(0,700),standardize = FALSE,raw.points = F)+ 
  annotate("text", x = mean(npk$x.rich), y = mean(npk$x.func), 
           label = "*",size=8)+ggtitle('NPK 0 NPK N')+theme_classic()

p3 <- leap.zig(ctl,type='cafe',xlim=c(0,15),ylim=c(0,700),standardize = FALSE,raw.points = T)+ 
  annotate("text", x = mean(ctl$x.rich), y = mean(ctl$x.func), 
           label = "*",size=8)+ggtitle('Control 0 Control N')+theme_classic()
p4 <- leap.zig(npk,type='cafe',xlim=c(0,15),ylim=c(0,700),standardize = FALSE,raw.points = T)+ 
  annotate("text", x = mean(npk$x.rich), y = mean(npk$x.func), 
           label = "*",size=8)+ggtitle('NPK 0 NPK N')+theme_classic()
...
p3 <- leap.zig(dat4,type='cafe', standardize = FALSE,raw.points = F)+ 
  annotate("text", x = mean(dat4$x.rich), y = mean(dat4$x.func), 
           label = "*",size=8)+ggtitle('badlau.de')+theme_classic()

p33 <- leap.zig(dat4,type='cafe', standardize = FALSE,raw.points = T)+ 
  annotate("text", x = mean(dat4$x.rich), y = mean(dat4$x.func), 
           label = "*",size=8)+ggtitle('badlau.de')+theme_classic()

grid_arrange_shared_legend(p3,p33,ncol=2,nrow=1)

# MATRIX OF EVERY GODDAMNED SITE FOR STUPID OOOOO FACTOR
npk$site_code<-as.factor(npk$site_code)
levels(npk$site_code)
dat1<-npk
