...
setwd("/Users/el50nico/Desktop/Academic/Manuscripts/2018_6 _NutNet_biomass/NutNet data")
...
nn <- read.csv("comb-by-plot-09-April-2018.csv", sep=",",header=T, strip.white=T)
# get a list of unique sites, continent, habitat, year_trt year and experiment type
unique.sites <- unique(nn[c("site_name","continent","habitat","year_trt","year","experiment_type")])
# add a column to count the number of sites
unique.sites$site_n<-"1"
#write to a new file
write.csv(unique.sites, "basics_1.csv", row.names=F)

# and a second dataset that summarizes sites, by only their max year
nn_agg <- read.csv("basics_1.csv", sep=",",header=T, strip.white=T)
# take only the max year of every site
nn_agg_2<-nn_agg %>% group_by(site_name) %>% top_n(1, year_trt)
write.csv(nn_agg_2, "basics_2.csv", row.names=F)
...
nn_agg2 <- read.csv("basics_1.csv", sep=",",header=T, strip.white=T)
...
nnr <- read.csv("basics_2.csv", sep=",",header=T, strip.white=T)
