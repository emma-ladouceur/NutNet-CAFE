...
quads <- read.csv("~/GRP GAZP Dropbox/Emma Ladouceur/_Projects/NutNet/Data/quads.csv" ,header=T,fill=TRUE,sep=",",na.strings=c(""," ","NA","NA ","na"))

quads <- quads %>% select(-Quadrant)

npk.effs <- npk.effs %>% left_join(quads)

head(npk.effs)
 
sloss.ps <- sloss.ps %>%
left_join(quads , by = c("site_code"))

sloss.ps.new <- sloss.ps %>% group_by(Biomass_R) %>%
  mutate( response="NPK", eff =  mean(sloss.trt.study + sloss.trt.global),
          eff_lower = quantile(sloss.trt.study + sloss.trt.global, probs=0.025),
          eff_upper = quantile(sloss.trt.study + sloss.trt.global, probs=0.975))  %>%
  dplyr::select(c(site_code, eff,eff_upper,eff_lower,Biomass_R)) %>% distinct()  
...
fig_5a <- ggplot() +
  geom_density_ridges(data = sloss.ps, #density ridges
                      ...
                      scale = 1, alpha = 0.3,
                      linetype = 0) +
  geom_point(data =npk.effs, aes(y = Biomass_R, x = sloss.trt.rate.p), #study-level effects
             colour= "#B40F20", shape=1, size = 2,  position = position_jitter(height = 0.02 )) +
  geom_point(data= sloss.ps.new,
             aes(x= eff, y= Biomass_R),  size=4, shape=5)+
  geom_errorbar(data = sloss.ps.new, aes(y = Biomass_R, xmin = eff_lower,
                                        xmax = eff_upper),
               width = 0, size = 0.5) +
...
sgain.ps.new <- sgain.ps %>% group_by(Biomass_R) %>%
  mutate( response="NPK", eff =  mean(sgain.trt.study + sgain.trt.global),
          eff_lower = quantile(sgain.trt.study + sgain.trt.global, probs=0.025),
          eff_upper = quantile(sgain.trt.study + sgain.trt.global, probs=0.975))  %>%
  dplyr::select(c(site_code, eff,eff_upper,eff_lower,Biomass_R)) %>% distinct()  

fig_5b <- ggplot() +
  geom_density_ridges(data = sgain.ps, #density ridges
                      aes(x = sgain.trt.study + sgain.trt.global, 
                          y = Biomass_R,
                      ), fill="#3B9AB2",
                      scale = 1, alpha = 0.3,
                      linetype = 0) +
  geom_point(data =npk.effs, aes(y = Biomass_R, x = sgain.trt.rate.p), #study-level effects
             colour= "#3B9AB2",, shape=1, size = 2,  position = position_jitter(height = 0.02 )) +
  geom_point(data= sgain.ps.new,
             aes(x= eff, y= Biomass_R),  size=4, shape=5)+
  geom_errorbar(data = sgain.ps.new, aes(y = Biomass_R, xmin = eff_lower,
                                         xmax = eff_upper),
                width = 0, size = 0.5) +
  geom_vline(xintercept = 0, lty = 2) +
  theme_bw(base_size=14) +
...
sl.ps.new <- sl.ps %>% group_by(Biomass_R) %>%
  mutate( response="NPK", eff =  mean(sl.trt.study + sl.trt.global),
          eff_lower = quantile(sl.trt.study + sl.trt.global, probs=0.025),
          eff_upper = quantile(sl.trt.study + sl.trt.global, probs=0.975))  %>%
  dplyr::select(c(site_code, eff,eff_upper,eff_lower,Biomass_R)) %>% distinct()  
...
fig_5c <- ggplot() +
  geom_density_ridges(data = sl.ps,
                      ...
                      scale = 1, alpha = 0.3,
                      linetype = 0) +
  geom_point(data =npk.effs, aes(y = Biomass_R, x = sl.trt.rate.p), #study-level effects
             colour= "#B40F20", shape=1, size = 2,  position = position_jitter(height = 0.02 )) +
  geom_point(data= sl.ps.new,
             aes(x= eff, y= Biomass_R),  size=4, shape=5)+
  geom_errorbar(data = sl.ps.new, aes(y = Biomass_R, xmin = eff_lower,
                                         xmax = eff_upper),
                width = 0, size = 0.5) +
  geom_vline(xintercept = 0, lty = 2) +
  theme_bw(base_size=14) +
...
sg.ps.new <- sg.ps %>% group_by(Biomass_R) %>%
  mutate( response="NPK", eff =  mean(sg.trt.study + sg.trt.global),
          eff_lower = quantile(sg.trt.study + sg.trt.global, probs=0.025),
          eff_upper = quantile(sg.trt.study + sg.trt.global, probs=0.975))  %>%
  dplyr::select(c(site_code, eff,eff_upper,eff_lower,Biomass_R)) %>% distinct()  


fig_5d <- ggplot() +
  geom_density_ridges(data = sg.ps, #density ridges
                      aes(x = sg.trt.study + sg.trt.global, 
                          y = Biomass_R,
                      ), fill="#3B9AB2",
                      scale = 1, alpha = 0.3,
                      linetype = 0) +
  geom_point(data =npk.effs, aes(y = Biomass_R, x = sg.trt.rate.p), #study-level effects
             colour= "#3B9AB2",, shape=1, size = 2,  position = position_jitter(height = 0.02 )) +
  geom_point(data= sg.ps.new,
             aes(x= eff, y= Biomass_R),  size=4, shape=5)+
  geom_errorbar(data = sg.ps.new, aes(y = Biomass_R, xmin = eff_lower,
                                         xmax = eff_upper),
                width = 0, size = 0.5) +
  geom_vline(xintercept = 0, lty = 2) +
  theme_bw(base_size=14) +
...
cde.ps.new <- cde.ps %>% group_by(Biomass_R) %>%
  mutate( response="NPK", eff =  mean(cde.trt.study + cde.trt.global),
          eff_lower = quantile(cde.trt.study + cde.trt.global, probs=0.025),
          eff_upper = quantile(cde.trt.study + cde.trt.global, probs=0.975))  %>%
  dplyr::select(c(site_code, eff,eff_upper,eff_lower,Biomass_R)) %>% distinct()  
...
View(npk.effs)
fig_5e <- ggplot() +
  geom_density_ridges(data = cde.ps, #density ridges
                      aes(x = cde.trt.study + cde.trt.global, 
                          y = Biomass_R,
                      ), fill="#F98400",
                      scale = 1, alpha = 0.3,
                      linetype = 0) +
  geom_point(data =npk.effs, aes(y = Biomass_R, x = cde.trt.rate.p), #study-level effects
             colour= "#F98400", shape=1, size = 2,  position = position_jitter(height = 0.02 )) +
  geom_point(data= cde.ps.new,
             aes(x= eff, y= Biomass_R),  size=4, shape=5)+
  geom_errorbar(data = cde.ps.new, aes(y = Biomass_R, xmin = eff_lower,
                                      xmax = eff_upper),
                width = 0, size = 0.5) +
  geom_vline(xintercept = 0, lty = 2) +
  theme_bw(base_size=14) +
  ...
  geom_text(data = cde.ps %>%
              ...
              ungroup() %>%
              distinct(Biomass_R, n_sites, .keep_all = T),
            aes(x=-100, y=Biomass_R,
